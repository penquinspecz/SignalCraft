...
try:
    ...
    raise SystemExit(f"invalid JOBINTEL_CANDIDATE_ID: {exc}") from exc
...
def _acquire_lock(timeout_sec: int = 0) -> None:
    ...
    while True:
        try:
            ...
            if timeout_sec == 0:
                raise SystemExit(f"Another run is already in progress (lock: {LOCK_PATH}).") from None
            if time.time() - start > timeout_sec:
                raise SystemExit(f"Timed out waiting for lock: {LOCK_PATH}.") from None
            time.sleep(2)
...
def _environment_fingerprint() -> Dict[str, Optional[str]]:
    try:
        ...
        try:
            platform_value = f"{platform.system()}-{platform.release()}"
        except Exception:
...
def _write_diff_summary(run_dir: Path, payload: Dict[str, Any]) -> None:
    ...
    for provider in sorted(provider_profile):
        ...
        for profile in sorted(profiles):
            ...
            lines.append(
                f"- {label}: new={counts.get('new', 0)} "
                f"changed={counts.get('changed', 0)} removed={counts.get('removed', 0)}"
            )
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def resolve_stage_order(
    *,
    ...
    stage_order: List[str] = ["scrape"]
    if scrape_only:
        return stage_order
    for provider in providers:
        stage_order.append(f"classify:{provider}")
        if not no_enrich:
            stage_order.append(f"enrich:{provider}")
        if ai:
            stage_order.append(f"ai_augment:{provider}")
        for profile in profiles:
            stage_order.append(f"score:{provider}:{profile}")
    return stage_order
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Write structured JSONL logs to state/runs/<run_id>/logs/run.log.jsonl "
            "(also enable via JOBINTEL_LOG_FILE=1)."
        ),
    )
    ...
    def _finalize(status: str, extra: Optional[Dict[str, Any]] = None) -> str:
        ...
        if max_ai_tokens_per_run > 0 and costs_payload["ai_estimated_tokens"] > max_ai_tokens_per_run:
            budget_errors.append(
                "ai_estimated_tokens="
                f"{costs_payload['ai_estimated_tokens']} > MAX_AI_TOKENS_PER_RUN={max_ai_tokens_per_run}"
            )
        if max_embeddings_per_run > 0 and costs_payload["embeddings_count"] > max_embeddings_per_run:
            budget_errors.append(
                "embeddings_count="
                f"{costs_payload['embeddings_count']} > MAX_EMBEDDINGS_PER_RUN={max_embeddings_per_run}"
            )
        ...
        logger.info(
            "PUBLISH_CONTRACT enabled=%s required=%s bucket=%s prefix=%s "
            "pointer_global=%s pointer_profiles=%s error=%s",
        ...
        if status == "success" and history_enabled:
            ...
            for profile in unique_profiles:
                ...
                logger.info(
                    "HISTORY_RETENTION profile=%s run_id=%s enabled=1 keep_runs=%d keep_days=%d "
                    "runs_kept=%d runs_pruned=%d daily_kept=%d daily_pruned=%d "
                    "identity_count=%d identity_map=%s provenance=%s run_pointer=%s daily_pointer=%s",
                    profile,
                    run_id,
                    history_keep_runs,
                    history_keep_days,
    ...
    def record_stage(name: str, fn: Callable[[], Any]) -> Any:
        ...
        stage_result = StageResult(status=status, outputs={}, duration_sec=duration_sec)
        stage_results[name] = stage_result
        telemetry["stages"][name] = {
            "duration_sec": stage_result.duration_sec,
    ...
    try:
        ...
        telemetry["stage_plan"] = planned_stages
        ai_required = args.ai
        ...
        if base_short:
                ...
                record_stage(
                    ...
                    lambda: _run(
                        build_ai_augment_command(
                            ...
                            repo_root=stage_context.workspace.repo_root,
                        ),
                        stage=current_stage,
                    ),
                )
            ...
            for profile in profiles:
                ...
                cmd = build_score_command(
                    ...
                    out_json=ranked_json,
                    out_csv=ranked_csv,
                    out_families=ranked_families,
                    out_md=shortlist_md,
                    min_score=args.min_score,
                    out_md_top_n=top_md,
                    semantic_scores_out=semantic_score_artifact_path(
                        run_id=run_id,
                        provider="openai",
                        profile=profile,
                        run_metadata_dir=stage_context.workspace.run_metadata_dir,
        ...
        if args.offline:
            ...
            scrape_mode = "AUTO"
        snapshot_write_dir: Optional[Path] = None
        if scrape_mode in {"LIVE", "AUTO"} and not args.snapshot_only:
            ...
            if env_snapshot_dir is None:
                ...
                snapshot_write_dir.mkdir(parents=True, exist_ok=True)
        scrape_cmd = build_scrape_command(
            python_executable=sys.executable,
            repo_root=stage_context.workspace.repo_root,
            scrape_mode=scrape_mode,
            providers=providers,
        ...
        stage_context = StageContext(
            ...
            providers=stage_context.providers,
            profiles=stage_context.profiles,
            config_paths=stage_context.config_paths,
            provenance=provenance_by_provider,
        )
        ...
        for provider in providers:
            ...
            record_stage(
                current_stage,
                lambda p=raw_path, o=labeled_path, stage_name=current_stage: _run(
                    build_classify_command(
                        python_executable=sys.executable,
                        repo_root=stage_context.workspace.repo_root,
                        in_path=p,
            ...
            if args.no_enrich:
                ...
                record_stage(
                    current_stage,
                    lambda p=labeled_path, o=enriched_path, stage_name=current_stage: _run(
                        build_enrich_command(
                            python_executable=sys.executable,
                            repo_root=stage_context.workspace.repo_root,
                            in_path=p,
            ...
            if args.ai:
                ...
                record_stage(
                    current_stage,
                    lambda p=enriched_path, o=ai_out_path, stage_name=current_stage: _run(
                        build_ai_augment_command(
                            python_executable=sys.executable,
                            repo_root=stage_context.workspace.repo_root,
                            in_path=p,
            ...
            for profile in profiles:
                ...
                if provider in providers and os.environ.get("AI_ENABLED", "0").strip() == "1":
                    ...
                    cmd = build_ai_insights_command(
                        python_executable=sys.executable,
                        repo_root=stage_context.workspace.repo_root,
                        provider=provider,
                        profile=profile,
                        ranked_path=ranked_json,
                        run_id=run_id,
                        prev_path=state_path if state_exists else None,
                    )
                    record_stage(current_stage, lambda cmd=cmd, stage_name=current_stage: _run(cmd, stage=stage_name))
                ...
                if (
                    ...
                    cmd = build_ai_job_briefs_command(
                        python_executable=sys.executable,
                        repo_root=stage_context.workspace.repo_root,
                        provider=provider,
                        profile=profile,
                        ranked_path=ranked_json,
                        run_id=run_id,
                        max_jobs=os.environ.get("AI_JOB_BRIEFS_MAX_JOBS", "10"),
                        max_tokens_per_job=os.environ.get("AI_JOB_BRIEFS_MAX_TOKENS", "400"),
                        total_budget=os.environ.get("AI_JOB_BRIEFS_TOTAL_BUDGET", "2000"),
                    )
                    record_stage(current_stage, lambda cmd=cmd, stage_name=current_stage: _run(cmd, stage=stage_name))
